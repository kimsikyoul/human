			...
			if timestring == bossTime[i]:
				if bossTimeString[i] != '99:99:99' or bossMungFlag[i] == True :
					if bossMungFlag[i] == True :
						if bossData[i][2] == '0' :
							information1 += ' - ' + bossData[i][0] + '(' + bossData[i][1] + '.' + bossData[i][5] + ') : ' + tmp_bossTime[i].strftime('%H:%M:%S') + ' @ ' + tmp_bossTime[i].strftime('%Y-%m-%d') + ' (No Input ' + str(bossMungCnt[i]) + 't)' + ' * ' + bossData[i][6] + '\n'
						else : 
							information1 += ' - ' + bossData[i][0] + '(' + bossData[i][1] + '.' + bossData[i][5] + ') : ' + tmp_bossTime[i].strftime('%H:%M:%S') + ' @ ' + tmp_bossTime[i].strftime('%Y-%m-%d') + ' (Fake Spawn ' + str(bossMungCnt[i]) + 't)' + ' * ' + bossData[i][6] + '\n'
					else:
						if bossData[i][2] == '0' :
							information1 += ' - ' + bossData[i][0] + '(' + bossData[i][1] + '.' + bossData[i][5] + ') : ' + bossTimeString[i] + ' @ ' + bossDateString[i] + ' (No Input ' + str(bossMungCnt[i]) + 't)' + ' * ' + bossData[i][6] + '\n'
						else : 
							information1 += ' - ' + bossData[i][0] + '(' + bossData[i][1] + '.' + bossData[i][5] + ') : ' + bossTimeString[i] + ' @ ' + bossDateString[i] + ' (Fake Spawn ' + str(bossMungCnt[i]) + 't)' + ' * ' + bossData[i][6] + '\n'
...
async def dbLoad():
	...
	if len(beforeBossData) > 1:	
		...
		LoadChk = 1
		print ("Boss Time Info. is not available")
	...
	async def boss_check(self):
				...
				if basicSetting[21] == "1":
					try:
						...
						if self.bot.voice_clients[0].is_connected() :
							await self.bot.get_channel(channel).send( '< I am back! >', tts=False)
							self.checker = True
							print("명치복구완료!")
					except:
		...
		while True:
			...
			if log_stream.getvalue().find("Awaiting") != -1:
				...
				log_stream.seek(0)
				await self.bot.get_channel(channel).send( '< Discord error! I will be back! >', tts=False)
				await dbSave()
				break
				...
				for i in range(fixed_bossNum):
					...
					if basicSetting[3] != '0' and fixed_bossFlag0[i] == False:
						if fixed_bossTime[i] <= priv0 and fixed_bossTime[i] > priv:
							fixed_bossFlag0[i] = True
							await self.bot.get_channel(channel).send("```" + fixed_bossData[i][0] + ' ' + basicSetting[3] + 'minutes ' + fixed_bossData[i][3] +' [' +  fixed_bossTime[i].strftime('%H:%M:%S') + ']```', tts=False)
							try:
								if basicSetting[21] == "1":
					...
					if basicSetting[1] != '0' and fixed_bossFlag[i] == False:
						if fixed_bossTime[i] <= priv and fixed_bossTime[i] > now and fixed_bossFlag0[i] == True :
							fixed_bossFlag[i] = True
							await self.bot.get_channel(channel).send("```" + fixed_bossData[i][0] + ' ' + basicSetting[1] + 'minutes ' + fixed_bossData[i][3] +' [' +  fixed_bossTime[i].strftime('%H:%M:%S') + ']```', tts=False)
							try:
								if basicSetting[21] == "1":
				...
				for i in range(bossNum):
					...
					if basicSetting[3] != '0' and bossFlag0[i] == False:
						if bossTime[i] <= priv0 and bossTime[i] > priv:
							bossFlag0[i] = True
							if bossData[i][6] != '' :
								await self.bot.get_channel(channel).send("```" + bossData[i][0] + ' ' + basicSetting[3] + 'minutes remaining ' + bossData[i][3] + " [" +  bossTimeString[i] + "]" + '\n<' + bossData[i][6] + '>```', tts=False)
								await message.channel.send("@everyone" + bossData[i][0] + "5mins!")
							else :
								await self.bot.get_channel(channel).send("```" + bossData[i][0] + ' ' + basicSetting[3] + 'minutes remaining ' + bossData[i][3] + " [" +  bossTimeString[i] + "]```", tts=False)
								await message.channel.send("@everyone" + bossData[i][0] + "5mins!")
							try:
								if basicSetting[21] == "1":
					...
					if basicSetting[1] != '0' and bossFlag[i] == False:
						if bossTime[i] <= priv and bossTime[i] > now and bossFlag0[i] == True:
							bossFlag[i] = True
							if bossData[i][6] != '' :
								await self.bot.get_channel(channel).send("```" + bossData[i][0] + ' ' + basicSetting[1] + 'minutes ' + bossData[i][3] + " [" +  bossTimeString[i] + "]" + '\n<' + bossData[i][6] + '>```', tts=False)
								await message.channel.send("@everyone" + bossData[i][0] + "1min!")
							else :
								await self.bot.get_channel(channel).send("```" + bossData[i][0] + ' ' + basicSetting[1] + 'minutes ' + bossData[i][3] + " [" +  bossTimeString[i] + "]```", tts=False)
								await message.channel.send("@everyone" + bossData[i][0] + "1min!")
							try:
								if basicSetting[21] == "1":
						...
						if (bossTime[i]+datetime.timedelta(days=-365)) <= aftr:
							if basicSetting[2] != '0' and basicSetting[22] != '0' and bossFlag[i] == True and bossFlag0[i] == True and bossMungFlag[i] == True :
								if int(basicSetting[17]) <= bossMungCnt[i] and int(basicSetting[17]) != 0:
									...
									bossMungCnt[i] = 0
									if bossData[i][2] == '0':
										await self.bot.get_channel(channel).send(f'```No Input Counts {basicSetting[17]} exceed! [{bossData[i][0]}] Del```', tts=False)
										print ('Auto No Input Counts limit exceed <' + bossData[i][0] + ' Deleted!>')
									else:
										await self.bot.get_channel(channel).send(f'```Fake Spawn Counts {basicSetting[17]} exceed! [{bossData[i][0]}] Del```', tts=False)
										print ('Auto Fake Spawn Counts limit exceed <' + bossData[i][0] + ' Deleted!>')
									#await dbSave()
									...
									if bossData[i][2] == '0':
										...
										await self.bot.get_channel(channel).send("```" +  bossData[i][0] + ' No Data.```', tts=False)
										embed = discord.Embed(
											description= '```Next Boss Time is ' + bossData[i][0] + ' ' + bossTimeString[i] + '.```',
											color=0xff0000
											)
										await self.bot.get_channel(channel).send(embed=embed, tts=False)
										try:
...
class mainCog(commands.Cog): 
	...
	async def menu_(self, ctx):
		if ctx.message.channel.id == basicSetting[7]:
			...
			command_list += ','.join(command[12]) + ' [#of winners] [IGN1] [IGN2]...\n'     #!사다리
			...
			command_list += ','.join(command[16]) + ' [TEXT]\n'     #!v
			command_list += ','.join(command[17]) + ' Boss Appearance Intervals\n'     #!리젠
			command_list += ','.join(command[18]) + ' Current Human L2M Bot Time\n'     #!time
			...
			command_list += ','.join(command[19]) + ' [Notice Detail]\n'     #!공지
			command_list += ','.join(command[20]) + '\n'     #!공지삭제
			command_list += ','.join(command[21]) + ' [Human L2M Bot Status]\n'     #!상태
			#command_list += ','.join(command[28]) + ' 사다리, 정산, 척살, 경주, 아이템\n'     #!채널설정
			#command_list += ','.join(command[42]) + ' 사다리, 정산, 척살, 경주, 아이템\n'     #!채널삭제
			command_list += ','.join(command[34]) + ' ※ Server Owner ONLY\n\n'     #서버나가기
			command_list += ','.join(command[22]) + '\n'     #보스탐
			command_list += ','.join(command[23]) + '\n'     #!보스탐
			command_list += '[Boss]DOWN or [Boss] DOWN 0000, 00:00\n'  
			#command_list += '[보스명] 컷 또는 [보스명] 컷 0000, 00:00\n'   
			command_list += '[Boss]FAKE or [Boss] FAKE 0000, 00:00\n'     
			command_list += '[Boss]EST or [Boss] EST 0000, 00:00(not working well...)\n' 
			command_list += '[Boss]DEL or [Boss] DEL\n'     
			...
			embed.add_field(
					...
					value= '```!down [boss] 00:00:00 !fake [boss] 00:00:00 !estimate [boss] 00:00:00  ex) !down lily 12:00:00, !fake lily 16:00:00, !estimate lily 20:00:00 ```'
					)
	...
	async def loadDB_(self, ctx):
		if ctx.message.channel.id == basicSetting[7]:
			...
			if LoadChk == 0:
				await ctx.send('<Loading Complete>', tts=False)
			else:
				await ctx.send('<No Boss Time Data>', tts=False)
		else:
			return
	...
	async def playText_(self, ctx):
		if ctx.message.channel.id == basicSetting[7]:
			...
			sayMessage = msg
			try:
				await MakeSound(ctx.message.author.display_name +'said, ' + sayMessage, './sound/say')
			except:
				await ctx.send( f"```음성파일 생성에 실패하였습니다.!(amazon polly 사용시 키 값을 확인하세요!)```")
				return
			await ctx.send("```< " + ctx.author.display_name + " >said \"" + sayMessage + "\"```", tts=False)
			try:
				if aws_key != "" and aws_secret_key != "":
	...
	async def currentTime_(self, ctx):
		if ctx.message.channel.id == basicSetting[7]:
			...
			embed = discord.Embed(
				title = 'Current Time is ' + curruntTime.strftime('%H') + 'h ' + curruntTime.strftime('%M') + 'm '+ curruntTime.strftime('%S')+ 's .',
				color=0xff00ff
				)
			await ctx.send( embed=embed, tts=False)
		else:
	...
	async def notice_(self, ctx):
		if ctx.message.channel.id == basicSetting[7]:
			...
			if len(msg) > 1:
				...
				await ctx.send( '< Notice posted >', tts=False)
			else:
	...
	async def botStatus_(self, ctx):
		if ctx.message.channel.id == basicSetting[7]:
			...
			await ctx.send( '< Human L2M Bot Status Changed! >', tts=False)
		else:
			return
					...
					if timestring == ouput_bossData[i][1]:
						if ouput_bossData[i][4] == '0' :
							if ouput_bossData[i][5] == 0 :
								...
								boss_information[cnt] = boss_information[cnt] + ouput_bossData[i][3] + ' ' + ouput_bossData[i][2] + ' : ' + ouput_bossData[i][0] + ' (No Input ' + str(ouput_bossData[i][5]) + 't)' + ' ' + ouput_bossData[i][6] + '\n'
						else : 
							if ouput_bossData[i][5] == 0 :
								boss_information[cnt] = boss_information[cnt] + ouput_bossData[i][3] + ' ' + ouput_bossData[i][2] + ' : ' + ouput_bossData[i][0] + ' ' + ouput_bossData[i][6] + '\n'
							else :
								boss_information[cnt] = boss_information[cnt] + ouput_bossData[i][3] + ' ' + ouput_bossData[i][2] + ' : ' + ouput_bossData[i][0] + ' (Fake ' + str(ouput_bossData[i][5]) + 't)' + ' ' + ouput_bossData[i][6] + '\n'
